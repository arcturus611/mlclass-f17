#%% Code for ML HW 4
# TODO: learn_kernel_.... function should be able to take var-lenght params? for var num of hyperparams?
#%%
from cvxpy import *
import numpy as np
...
#%%
def solve_for_alpha(K, lam1, lam2, y, err_fn):
    n = len(y) 
    alpha = Variable(n)
    D = -1*np.eye(n) + np.eye(n, k = 1)
    DK = D.dot(K)
    objective = Minimize(sum_squares(K*alpha - y) + lam1*tv(K*alpha) + lam2*quad_form(alpha, K))
...
#%% 
def learn_exp_kernel_by_cv(lambda_1, lambda_2, gamma, x, y, fx, err_fn):
    ...
    for i in range(len(x)):
        ...
        #%% 
        alpha_hat = solve_for_alpha(K_tr, lambda_1, lambda_2, y_tr, err_fn)
...
if __name__ == '__main__':
    ...
    lambda_1 = np.linspace(.1, 10, 5) 
    lambda_2 = np.linspace(.1, 10, 5)
    heu_gamma = gamma_choose_heuristic(x)
    print("heuristic computed value is {}".format(heu_gamma))
    gamma = np.linspace(heu_gamma-1, heu_gamma+20, 5) #400
    total_err = np.empty((len(lambda_1), len(lambda_2), len(gamma)))
    err_fn = "tv"

    #%%learn the kernel for each hyperparam pair using loocv on input data
    for i in np.arange(len(lambda_1)):
        print("i = {}".format(i))
        for j in np.arange(len(lambda_2)):
            print("## j in {}".format(j))
            for k in np.arange(len(gamma)):
                print("~~~ k = {}".format(k))
                total_err[i, j, k] = learn_exp_kernel_by_cv(lambda_1[i], lambda_2[j], gamma[k], x, y, fx, err_fn)
                
    #%% based on total_err, we find the best lambda_1, d
    (lambda_1_min_idx, lambda_2_min_idx, gamma_min_idx)= np.unravel_index(total_err.argmin(), total_err.shape)
    best_lambda_1 = lambda_1[lambda_1_min_idx]
    best_lambda_2 = lambda_2[lambda_2_min_idx]
    best_gamma = gamma[gamma_min_idx]
    print("(best_lambda_1 = {},  best_lambda_2 = {}, best_gamma = {}), and equals total_err[best_lambda_1, best_lambda_2, best_gamma]= {}".format(best_lambda_1, best_lambda_2, best_gamma, total_err[lambda_1_min_idx, lambda_2_min_idx, gamma_min_idx]))
    
    #%% Now we build the predictor
    K_learnt = generate_exp_kernel(x, x, best_gamma)
    alpha_learnt = solve_for_alpha(K_learnt, best_lambda_1, best_lambda_2, y, err_fn)
