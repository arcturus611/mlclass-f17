...
if __name__ == '__main__':
    ...
    for kk in np.arange(num_bootstrap):
        ...
        K_learnt = generate_poly_kernel(x_sampled, x_sampled, best_d)
        alpha_learnt = solve_for_alpha(K_learnt, best_lam, y_sampled)
        

        for i in range(num_new_test):
            fxhat_new_test[kk, i] = np.inner(alpha_learnt, generate_poly_kernel(new_test[i], x_sampled, best_d))
    ...
    #%%
    orig_best_kernel = generate_poly_kernel(x, x, best_d)
    orig_best_alpha = solve_for_alpha(orig_best_kernel, best_lam, y)
    fxhat_orig_new_test = np.empty(num_new_test)
    for i in range(num_new_test):
        fxhat_orig_new_test[i] = np.inner(orig_best_alpha, generate_poly_kernel(new_test[i], x, best_d))
    ...
    plt.title('Bootstrapping (poly kernel)')